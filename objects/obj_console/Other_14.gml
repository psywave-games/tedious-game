...
if (text_part[0] == commands[command_1.create]) {
					...
					if (!object_exists(object_to_try_and_edit)) { // object doesnt exist 
						...
						if (instance_number(object_to_try_and_edit) == 0) { // object doesnt exist 
							...
							with (object_to_try_and_edit) { // many things might have this 
								...
								if (string_length(string_digits(other.text_part[3])) == string_length(string_replace("-", "", other.text_part[3])) || string_length(string_digits(other.text_part[3])) == string_length(other.text_part[3])) { // is int
				...
				if (!object_exists(object_to_try_and_access)) { // object doesnt exist 
					...
					if (instance_number(object_to_try_and_access) == 0) { // object doesnt exist 
						...
						for (var i = 0; i < view_active_count; i++){
							if (view_title[i] == text_part[1]+"."+text_part[2]) {
								view_active_count -= 1
								remove = true
								break
							}	
						...
						/// delete this view
						if remove {
							for (i = i; i < view_active_count; i++){
								view_text[i] = view_text[i+1]
								view_obj[i] = view_obj[i+1]
								view_recalculate[i] = view_recalculate[i+1]
								view_title[i] = view_title[i+1]
								view_destroyed_frame[i] = view_destroyed_frame[i+1]
								view_next_insert[i] = view_next_insert[i+1]
								view_intro_highlight_frame[i] = view_intro_highlight_frame[i+1]
								view_type[i] = view_type[i+1]
								view_min[i] = view_min[i+1]
								view_max[i] = view_max[i+1]
							}
						}
						/// include this view
						else with (object_to_try_and_access) {
							...
							other.view_max[other.view_active_count] = new_value
							 ...
							 // 0=int, 1=str, 2=array, 3=destroyed, 4=unknown
							if (is_real(new_value)) {
								other.view_type[other.view_active_count] = 0
							}
							else if (is_string(new_value)) {
								other.view_type[other.view_active_count] = 1
							}
							else if (is_array(new_value)) {
								other.view_type[other.view_active_count] = 2
							}
							else {
								other.view_type[other.view_active_count] = 4	
							}
	...
	#endregion
} else if (text_part[0] == commands[command_1.jooj]) {
	#region game
		var parse_parts = array_length_1d(text_part)
	...
	#region console
		...
		if (parse_parts == 1) { // object doesnt exist 
			...
			if (text_part[1] == "clear_screen") {
				...
				event_user(1); 
				temp_post_message = "modified by Rodrigo  -  rodrigo.dornelles.me"
				event_user(1);
