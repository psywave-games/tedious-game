	...
	for (var i = 0; i < view_active_count; ++i) {
		...
		switch (view_type[i]) {
		    ...
		    case 2: // array
		        draw_set_colour(view_text_value_col)
				draw_set_halign(fa_left)
				draw_text(view_pos_area_x1-15, my_area_height-view_data_height+5, string(view_last_data[i]))
		    break;
		    case 3: // destroyed
				#region destroyed
					draw_set_colour(view_text_value_col)
		    ...
		    break;
			case 4: // unknown
				var new_value = is_undefined(view_last_data[i])? "undefined": (typeof(view_last_data[i]) + ":  " + string(view_last_data[i]))
		        draw_set_colour(view_text_value_col)
				draw_set_halign(fa_left)
				draw_text(view_pos_area_x1-15, my_area_height-view_data_height+5, new_value)
			break;
		}
