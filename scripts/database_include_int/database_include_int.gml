...
if _object == database_global begin
	var _variable_value = round(variable_global_get(_variable_name))

	if not _save then
		_variable_value = round(ini_read_real(database_global, _variable_name, _variable_value))
...
#region DATABASE OBJECT
	...
	var _variable_value = round(variable_instance_get(_object, _variable_name))
	var _ini_section = string_delete(_object_name, 1, 4)
	var _ini_keyname = _variable_name
	_ini_section = string_replace(_ini_section, "_", " ")

	if not _save then
		_variable_value = round(ini_read_real(_ini_section, _ini_keyname, _variable_value))
